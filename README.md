...
### 3. VT控制进程信号监控 (优化)
- **精准监控**：仅监控VT控制进程的信号活动，不再监控所有会话进程
- **VT信号专注**：特别关注VT相关的Release和Acquire信号
- **控制进程跟踪**：显示VT控制进程的详细信息（PID、命令、用户）
- **生命周期监控**：检测VT控制进程的终止和重启
...
- **VT控制进程变化**：每2秒检查VT控制进程的变化
- **VT控制进程信号活动**：每3秒检查VT控制进程的信号统计
